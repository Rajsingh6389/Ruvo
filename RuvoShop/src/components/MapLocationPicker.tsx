...
async function reverseGeocodeGoogle(lat: number, lng: number): Promise<Partial<LocationResult>> {
  try {
    ...
    console.log('[GoogleGeocoding] Requesting reverse geocode for:', lat, lng);
    const res  = await fetch(url);
    const json = await res.json();
    console.log('[GoogleGeocoding] Response status:', json.status, json.error_message ? `Error: ${json.error_message}` : '');
    ...
    if (json.status === 'OK' && json.results?.length) {
      ...
      console.log('[GoogleGeocoding] Resolved address:', result.formattedAddress);
      return result;
    }
    console.warn('[GoogleGeocoding] Web API non-OK status (' + json.status + '). Falling back to native device geocoder...');
  } catch (err) {
    console.error('[GoogleGeocoding] Web API fetch error:', err);
  }
  ...
  try {
    ...
    if (places && places.length > 0) {
      ...
      console.log('[NativeGeocoding Fallback] Resolved:', fallbackResult.formattedAddress);
      return fallbackResult;
    }
  } catch (expoErr) {
    console.error('[NativeGeocoding Fallback Error]:', expoErr);
  }
  return {};
}

const DEFAULT_REGION: Region = {
  latitude: 20.5937, longitude: 78.9629, latitudeDelta: 0.01, longitudeDelta: 0.01,
};
...
export const MapLocationPicker: React.FC<MapLocationPickerProps> = ({
  ...
  const fetchAndGoToCurrentLocation = useCallback(async () => {
    ...
    try {
      ...
      if (status !== 'granted') {
        console.warn('[MapLocationPicker] Location permission denied');
        setLocatingUser(false);
        return;
      }
      const pos = await Location.getCurrentPositionAsync({ accuracy: Location.Accuracy.High });
      console.log('[MapLocationPicker] Fetched user current location:', pos.coords.latitude, pos.coords.longitude);
      ...
      console.warn('[MapLocationPicker] Error getting current GPS position:', err);
    } finally {
      setLocatingUser(false);
    }
  }, [geocodeAndPreview]);
  ...
  const handleRegionChangeComplete = useCallback((r: Region) => {
    console.log('[MapView] Region change complete:', r.latitude, r.longitude);
  ...
  const handlePlaceSelected = useCallback((data: any, detail: any = null) => {
    console.log('[GooglePlacesAutocomplete] Place selected:', data?.description, 'Detail:', detail?.geometry?.location);
    const loc = detail?.geometry?.location;
    if (!loc) {
      console.warn('[GooglePlacesAutocomplete] Missing location in details:', detail);
      return;
    }
    const newRegion: Region = {
