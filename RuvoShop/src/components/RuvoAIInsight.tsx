...
/** Shared RuVo AI surface for recommendations and operational suggestions. */
...
export const RuVoAIInsight = ({ title, detail, style, ...props }: ViewProps & { title: string; detail: string }) => {
  ...
  return <View {...props} style={[styles.card, { borderColor: colors.ai, backgroundColor: colors.primarySoft }, style]}>
    <RuVoAIIcon /><View style={styles.copy}><Text style={[styles.title, { color: colors.textPrimary }]}>{title}</Text><Text style={[styles.detail, { color: colors.textSecondary }]}>{detail}</Text></View>
  </View>;
};
