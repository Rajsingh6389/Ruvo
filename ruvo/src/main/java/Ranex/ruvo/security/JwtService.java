...
public class JwtService {
 ...
 public String create(User user, String sessionId) { Instant now=Instant.now(); String subject = user.getMobileNumber(); var builder = Jwts.builder().subject(subject).claim("role", user.getRole().name()).issuedAt(Date.from(now)).expiration(Date.from(now.plusMillis(expiration))); if (sessionId != null) { builder.claim("sessionId", sessionId); } return builder.signWith(key).compact(); }
